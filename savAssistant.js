...
const oauth2Client = new google.auth.OAuth2(
  ...
  // Add this function before createDraft
...
async function processQueue() {
  ...
  while (requestQueue.length > 0) {
    const { formData, responseMessage, resolve, reject } = requestQueue.shift();
    try {
      const result = await createDraftImplementation(formData, responseMessage);
      resolve(result);
    } catch (error) {
      reject(error);
    }
...
// Rename existing createDraft implementation
async function createDraftImplementation(formData, responseMessage) {
  // Move existing createDraft code here
  try {
    ...
    const cleanedMessage = cleanResponseMessage(responseMessage);
      ...
      <html>
        <body style="font-family: Altivo, sans-serif; background-color: #f4f4f4; padding: 20px;">
          ...
          <img src="https://www.botcup.fr/wp-content/uploads/2024/12/LOGOTYPE-Noir.png" alt="Botcup" style="width: 100px; height: auto; display: block; margin: 0 auto 20px;">
            ...
            <div style="white-space: pre-wrap;">${cleanedMessage}</div>
            <p>Cordialement,</p>
...
// New createDraft function that uses the queue
async function createDraft(formData, responseMessage) {
  return new Promise((resolve, reject) => {
    requestQueue.push({ formData, responseMessage, resolve, reject });
    processQueue();
  });
}

  async function handleSav(req, res) {
